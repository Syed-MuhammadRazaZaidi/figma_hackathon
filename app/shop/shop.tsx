...
export default function Home() {
  ...
  const ProductCard = memo(({ food }: { food: FoodItem }) => (
    <Link href={`/shop/${food.name}`} className="rounded-lg shadow-md overflow-hidden transform transition-transform hover:scale-105">
            ...
            {categories.map(({ id, label }) => (
              <div key={id} className="flex items-center gap-3">
                <Input
                  ...
                  className="w-4 h-4 text-[#FF] rounded"
                />
            ...
            {sortOptions.map((option) => (
              <div key={option.value} className="flex items-center gap-3">
                <Input
                  ...
                  className="w-4 h-4 text-[#FF] rounded"
                />
                <label htmlFor={option.value} className="text-gray-700 hover:text-[#FF9F0D] cursor-pointer">
                  {option.label}
                </label>
              </div>
            ))}
    ...
    <div className="bg-gray-50 min-h-screen py-8">
      <div className="container mx-auto px-4 flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-3/4">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-8 p-4 rounded-lg shadow-sm">
